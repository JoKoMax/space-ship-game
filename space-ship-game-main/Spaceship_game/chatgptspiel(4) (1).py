...
pygame.display.set_caption("Bewegungs- und Schießspiel")
...
# UI
start_button = Button(
    screen, screen_width/2-150, screen_height-400, 300, 150, text='Start',
...
def show_menu():
    screen.fill(WHITE)
    title_text = font.render("Space ship game", True, BLACK)
    #start_text = font.render("Drücke ENTER, um zu starten", True, BLACK)
...
# Upgrade-Baum anzeigen
def show_upgrades_fkt():
    #screen.fill(WHITE)
    upgrade_text = font.render("Upgrades", True, BLACK)
    #coin_text = font.render(f"Münzen: {coins}", True, BLACK)
    screen.blit(upgrade_text, (screen_width // 2 - upgrade_text.get_width() // 2, 50))
    #screen.blit(coin_text, (10, 10))

    y_offset = 150
    for upgrade, data in upgrades.items():
        upgrade_info = f"{upgrade.capitalize()} (Level {data['level']}/{data['max_level']}) - Kosten: {data['base_cost']} Münzen"
        upgrade_text = font.render(upgrade_info, True, BLACK)
        screen.blit(upgrade_text, (screen_width // 2 - upgrade_text.get_width() // 2, y_offset))
        y_offset += 50
...
def buy_upgrade(upgrade):
    global coins, player_speed, player_health, bullet_speed, shoot_cooldown, bullet_width, bullet_height, player_new_health, bullet_count, gun_count
    if coins >= upgrades[upgrade]["base_cost"] and upgrades[upgrade]["level"] < upgrades[upgrade]["max_level"]:
        coins -= upgrades[upgrade]["base_cost"]
        upgrades[upgrade]["level"] += 1
        if upgrade == "speed":
            player_speed += 2
        elif upgrade == "health":
            player_new_health += 5
        elif upgrade == "bullet_speed":
            bullet_speed += 2
        elif upgrade == "fire_rate":
            shoot_cooldown = max(0.1, shoot_cooldown - 0.1)  # Reduziert die Schuss-Cooldown-Zeit
